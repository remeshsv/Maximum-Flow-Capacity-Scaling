...
// For Fall 2016 Lab2, following changes were made:
//   1. Capacity Scaling is introduced.
...
int scalingFactor; //Capacity Scaling factor
...
// Returns 1 if found, otherwise 0.
{
	...
	for (u = 0; u<n; u++)
		...
		while (head != tail)
		{
			...
			{
				v = edgeTab[i].head;
				if (color[v] == WHITE && edgeTab[i].capacity - edgeTab[i].flow >= scalingFactor)
				{
					enqueue(v);
					pred[v] = u;
					predEdge[v] = i;
...
int max_flow(int source, int sink)
{
	...
	for (; scalingFactor > 0;)
	{
		std::cout << "Scaling Parameter is " << scalingFactor << endl;
		...
		while (bfs(source, sink))
		{
			...
			//std::cout << increment << " " << scalingFactor << endl;
			//if (increment >= scalingFactor)
			{
...
void read_input_file()
{
	...
	std::cout << "Number of nodes: " << n << endl << "Number of edges: " << inputEdges << endl;
	...
	{
		//scanf_s("%d %d %d", &tail, &head, &capacity);
		std::getline(inp, str);
		std::istringstream iss(str);
		iss >> tail;
		iss >> head;
		...
		//Finding Max capacity among edges
	...
	scalingFactor = 1;
	for (; scalingFactor < maxcapacity;)
	{
		scalingFactor = scalingFactor * 2;
	}
	std::cout << scalingFactor << "  " << maxcapacity << endl;
	...
	//stopCPU = CPUtime();
	//printf("qsort CPU %f\n", stopCPU - startCPU);
	//std::cout << edgeTab[1200].tail << endl;
	if (n <= 20)
	{
		printf("Sorted edges:\n");
		dumpEdges(workingEdges);
